...
public class JsonSerializer_ModelOptionParts implements JsonDeserializer<OptionPartsInfoList> {

	@Override
	public OptionPartsInfoList deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			throws JsonParseException {
		// テクスチャ名を取得
		JsonObject jsonobject = JsonUtils.getJsonObject(json, "entry");

		// 追加パーツ情報を作成
		OptionPartsInfoList result = new OptionPartsInfoList();
		// 追加パーツのテクスチャ名を設定
		...
		while(flag){
			...
			try{
				...
				// オブジェクトを作成
				OptionPartsInfo newparts = new OptionPartsInfo(
