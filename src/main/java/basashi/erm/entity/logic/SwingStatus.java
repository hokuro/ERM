...
public class SwingStatus {
	 ...
	 */
	public void onEntityUpdate(EntityERMBase pEntity) {
	 ...
	 */
	public void onUpdate(EntityERMBase pEntity) {
		prevSwingProgress = swingProgress;
		if (attackTime > 0) {
			attackTime--;
		}
		...
		if (isUsingItem()) {
			ItemStack itemstack;
			try{
				itemstack = pEntity.getInventory().getStackInSlot(index);
			...
			if (itemstack != itemInUse) {
				...
				if (--itemInUseCount <= 0 && lrentity != null) {
					onItemUseFinish(pEntity.mobAvatar);
				}
			}
		}
	}
	 ...
	 */
	public ItemStack getItemStack(EntityERMBase pEntity) {
		if (index > -1) {
			return pEntity.getInventory().getStackInSlot(index);
		}
		return null;
	}
